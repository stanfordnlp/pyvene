...
ONE_MLP_WITH_W1_OUT_RUN = lambda w1_out, mlp : \
    mlp.mlp.h[0].act(w1_act) @ mlp.score.weight.T

ONE_MLP_WITH_W1_ACT_RUN = lambda w1_act, mlp : \
    w1_act @ mlp.score.weight.T
...
def GPT2_SELF_ATTENTION_RUN(self_attn, hidden_states, i, CACHE_ACTIVATIONS, INTERVENTION_ACTIVATIONS):
    query, key, value = self_attn.c_attn(hidden_states).split(self_attn.split_size, dim=2)
    ...
    head_query = DO_GPT2_INTERVENTION(f"{i}.head_query_output", head_query, INTERVENTION_ACTIVATIONS)
    CACHE_ACTIVATIONS[f"{i}.head_query_output"] = head_query
    head_key = DO_GPT2_INTERVENTION(f"{i}.head_key_output", head_key, INTERVENTION_ACTIVATIONS)
    CACHE_ACTIVATIONS[f"{i}.head_key_output"] = head_key
    head_value = DO_GPT2_INTERVENTION(f"{i}.head_value_output", head_value, INTERVENTION_ACTIVATIONS)
    ...
    head_attention_value_output = DO_GPT2_INTERVENTION(
        f"{i}.head_attention_value_output", head_attention_value_output, INTERVENTION_ACTIVATIONS)
...
def GPT2_BLOCK_RUN(block, hidden_states, i, CACHE_ACTIVATIONS, INTERVENTION_ACTIVATIONS):
    ...
    attn_outputs = GPT2_SELF_ATTENTION_RUN(
        block.attn, hidden_states, i, 
        CACHE_ACTIVATIONS, INTERVENTION_ACTIVATIONS
    )
