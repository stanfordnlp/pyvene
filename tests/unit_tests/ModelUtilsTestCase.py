...
class ModelUtilsTestCase(unittest.TestCase):
    ...
    def test_scatter_neurons_positive(self):
        ...
        golden_output[:, 0:2, 1:3, :] = replacing_tensor_input[:, 0:2, 0:2, :].permute(0, 2, 1, 3)
        
        tensor_output = scatter_neurons(tensor_input, 
    ...
    def test_scatter_gathered_neurons_positive(self):
        ...
        gathered_replacing_tensor_input = gather_neurons(replacing_tensor_input, 
                                                         "h.pos", 
                                                         ([[1,2]] * 2, [[0,1]] * 2))
        # batch_size, seq_len, emb_dim
        tensor_input = torch.rand((2, 5, 6))
        golden_output = tensor_input.clone().view((2, 5, 3, 2))
        golden_output[:, 0:2, 1:3, :] = gathered_replacing_tensor_input.permute(0, 2, 1, 3)
        tensor_output = scatter_neurons(tensor_input, 
...
def suite():
    ...
    suite.addTest(
        ModelUtilsTestCase(
            "test_scather_neurons_positive"
        )
    )
    suite.addTest(
        ModelUtilsTestCase(
            "test_scatter_gathered_neurons_positive"
        )
    )
    return suite
